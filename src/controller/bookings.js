...
export const reserveSeat = async (req, res) => {
  try {
    ...
    if (!eventId || !seatIds?.length)
      return res.status(400).json({ message: 'eventId and seatIds required' });
...
// Confirm booking (debit wallet + confirm seats)
export const confirmBooking = async (req, res) => {
  try {
    ...
    // Idempotency check
    const existingBooking = await Booking.findOne({ idempotencyKey: key });
    if (existingBooking)
    ...
    for (const seatId of seatIds) {
      ...
      if (String(seat.reservedBy) !== String(req.user._id))
        throw new Error(`Seat ${seat.seatNumber} reserved by another user`);
      if (seat.reservationExpiry < now)
        throw new Error(`Reservation for seat ${seat.seatNumber} has expired`);
