...
export const seatsCreation =  async (req, res) => {
  try {
    ...
    // classes: [{ class: 'economy', rows: ['A','B','C'], price: 500000 }]
...
export const adminCancelBooking =  async (req, res) => {
  ...
  try {
    ...
    if (user && booking.paymentStatus === 'paid') {
      ...
      user.wallet.transactions.push({ type: 'refund', amount: booking.totalAmount, description: `Admin refund for booking #${booking._id}` });
...
export const dashboardStats =  async (req, res) => {
  const [totalUsers, totalBookings, confirmedBookings, cancelledBookings, events] = await Promise.all([
